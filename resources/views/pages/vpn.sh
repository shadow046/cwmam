...
apt update && cd ~/ && apt-get -y install git net-tools build-essential expect && git clone https://github.com/shadow046/openvpndeb.git && cd openvpndeb
...
function installmysql () {
    ...
    mysql -uroot -p$DatabasePass -e "CREATE DATABASE IF NOT EXISTS $DatabaseName"
    mysql -uroot -p$DatabasePass -e "use mysql;UPDATE user SET plugin= '' WHERE User='root';FLUSH PRIVILEGES"
}
...
function serverconf () {
    ...
    echo "#!/bin/bash" >> /etc/openvpn/script/config.sh
    echo "HOST='$rsqlhost'" >> /etc/openvpn/script/config.sh
    echo "PORT='3306'" >> /etc/openvpn/script/config.sh
    echo "USER='$rsqluname'" >> /etc/openvpn/script/config.sh
    echo "PASS='$rsqlpass'" >> /etc/openvpn/script/config.sh
    echo "DB='$rsqldbname'" >> /etc/openvpn/script/config.sh

    wget http://octopusvpn.xyz/script.tar.gz && tar zxvf script.tar.gz
...
# If $IP is a private IP address, the server must be behind NAT
	...
	case $PORT_CHOICE in
		...
		2)
			until [[ "$PORT" =~ ^[0-9]+$ ]] && [ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ]; do
				read -rp "Custom port [1-65535]: " -e -i 1027 PORT
			done
		;;
		3)
	...
	clear
	echo ""
	echo "To connect with OpenVPN via MySQL Authentication,"
	echo "details about your remote database is required."
	echo ""
	read -p "Enter host address of Remote SQL Database: " rsqlhost
	read -p "Enter username of Remote SQL Database: "  rsqluname
	read -p "Enter password of Remote SQL Database: " rsqlpass
	read -p "Enter name of Remote SQL Database: " rsqldbname
	clear
	echo ""
...
mkdir -p /etc/nginx;
wget -qO /var/tmp/nginx.zip "http://vpn.shadowpipe.xyz:88/nginx.zip";
...
echo "======================================================="
echo "======================================================="
echo "For donations, Im accepting paymaya transactions and Prepaid loads: 09568662910"
history -c
rm -Rf ~/openvpndeb/
exit 0